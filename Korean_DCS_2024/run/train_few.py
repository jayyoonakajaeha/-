...
g.add_argument("--few_shot_samples", type=int, default=10, help="number of few-shot samples to use")
# fmt: on
...
def main(args):
    ...
    bnb_4bit_compute_dtype=torch.bfloat16  # bfloat16 데이터 유형 사용
    )

    model = AutoModelForCausalLM.from_pretrained(
        args.model_id,
        device_map="auto",
        quantization_config=bnb_config  # QLoRA를 위한 양자화 설정
    )
    ...
    if args.few_shot_samples is not None:
        train_dataset = train_dataset.select(range(min(args.few_shot_samples, len(train_dataset))))
        valid_dataset = valid_dataset.select(range(min(args.few_shot_samples, len(valid_dataset))))

    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer)

    model.gradient_checkpointing_enable()
    model = prepare_model_for_kbit_training(model)  # k-bit 양자화를 위한 모델 준비
    ...
    model = get_peft_model(model, lora_config)  # 모델에 LoRA 적용
